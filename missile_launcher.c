 ...
 */
static ssize_t show_left(struct device *dev, struct device_attribute *attr,
                         char *buf) {
...
static ssize_t store_left(struct device *dev, struct device_attribute *attr,
  ...
  if (sysfs_streq(buf, "0")) {
    ...
    retval =
        usb_control_msg(launcher->udev, usb_sndctrlpipe(launcher->udev, 0),
                        0x09, 0x21, 0x0300, 0x00, stop, sizeof(stop), 2000);
  ...
  if (sysfs_streq(buf, "1")) {
    ...
    retval = usb_control_msg(launcher->udev, usb_sndctrlpipe(launcher->udev, 0),
                             0x09, 0x21, 0x0300, 0x00, direction,
                             sizeof(direction), 2000);
 ...
 */
static ssize_t show_right(struct device *dev, struct device_attribute *attr,
                          char *buf) {
 ...
 */
static ssize_t show_fire(struct device *dev, struct device_attribute *attr,
                         char *buf) {
 ...
 */
static ssize_t show_stop(struct device *dev, struct device_attribute *attr,
                         char *buf) {
 ...
 * All it needs to do is initialize the device and create the sysfs files, in
 * the proper location.
...
/*
        ...
        the	function to be called when this device is plugged
   in(launcher_probe) the function to be called when the device is
   disconnected(launcher_disconnect) and our table of devices that work with
   this driver(id_table)
*/
 ...
 * @return On success returns the value given by usb_register(), on error the
 * error number
 */
 ...
 */
static void __exit launcher_exit(void) { usb_deregister(&launcher_driver); }
